...
export const FileUpload = ({ onFileUploaded, disabled }: FileUploadProps) => {
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      toast.error("الحد الأقصى للملف 5MB.");
      return;
    }

    if (!file.type.startsWith("image/") && file.type !== "application/pdf") {
      toast.error("ارفع صورة أو ملف PDF فقط.");
      return;
    }

    const ext = file.name.split(".").pop() || "png";
    const fileName = `chatbot/${Date.now()}-${crypto.randomUUID()}.${ext}`;
    ...
    if (error || !data) {
      toast.error("تعذر رفع الملف حالياً.");
      return;
    }
